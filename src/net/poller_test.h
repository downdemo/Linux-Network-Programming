...
template <template <typename> class Poller>
  requires(is_same_template_v<Poller, PollerEpoll> ||
           ...
           is_same_template_v<Poller, PollerSelect>)
class PollerTCPServer : public Poller<PollerTCPServer<Poller>> {
 public:
  PollerTCPServer(std::string_view ip,
                  uint16_t port,
...
template <template <typename> class Poller>
  requires(is_same_template_v<Poller, PollerEpoll> ||
           is_same_template_v<Poller, PollerPoll> ||
           is_same_template_v<Poller, PollerSelect>)
class PollerTCPClient : public Poller<PollerTCPClient<Poller>> {
 public:
  PollerTCPClient(std::string_view ip,
                  uint16_t port,
                  uint64_t timeout_millsec) {
    client_ =
